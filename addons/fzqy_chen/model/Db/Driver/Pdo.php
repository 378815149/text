...
class Pdo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                $config['dsn'] = $config['dsn']? $config['dsn']:"mysql:dbname={$config['database']};host={$config['hostname']};port={$config['hostport']};charset={$config['charset']}";
