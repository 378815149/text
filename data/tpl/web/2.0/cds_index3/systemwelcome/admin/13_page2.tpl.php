...
<ul class="we7-page-tab">
	<li <?php  if($op=='list') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('page2',array('op'=>'list'))?>">列表</a></li>
	<li <?php  if($op=='modify') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('page2',array('op'=>'modify'))?>">添加</a></li>
</ul>
<?php  if($op == 'list') { ?>
<table class="table we7-table table-hover vertical-middle table-manage">
	...
	<tr>
		...
		<td>
			...
			<a href="<?php  echo $this->createWebUrl('page2',array('id'=>$l['id'],'op'=>'modify'))?>" >修改</a>
			<a href="<?php  echo $this->createWebUrl('page2',array('id'=>$l['id'],'op'=>'del'))?>"  onclick="return confirm('确定要删除吗？')">删除</a>
			</div>
		</td>
	</tr>
	<?php  } } ?>
...
<div class="clearfix">
	<form class="we7-form" method="post" action="" enctype="multipart/form-data" id="sham_form">
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" id="content" name="content" placeholder="请输入内容" value="<?php  echo $tpl_data['content'];?>">
			</div>
		</div>
